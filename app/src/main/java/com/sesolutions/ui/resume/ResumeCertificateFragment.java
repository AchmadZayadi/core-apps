...
public class ResumeCertificateFragment extends VideoHelper implements View.OnClickListener, OnLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {
   ...
   /* @Override
    public void onResume() {
        ...
        if(Constant.backresume==Constant.FormType.CREATE_RESUME_CERTIFICATE || Constant.backresume==Constant.FormType.CREATE_RESUME_CERTIFICATE_EDIT){
    ...
    @Override
            ...
            try {
                if (isNetworkAvailable(context)) {
                   try {
                        ...
                        HttpRequestVO request = new HttpRequestVO(Constant.CREDIT_RESUME_CERTIFICATE);
        ...
        int resumeid;
        public static ResumeCertificateFragment newInstance(OnUserClickedListener<Integer, Object> parent, String selectedScreen, int resumid) {
            ResumeCertificateFragment frag = new ResumeCertificateFragment();
    ...
    public boolean onItemClicked(Integer object1, Object object2, int postion) {
        switch (object1){
            ...
            case Constant.Events.CLICKED_HEADER_EDIT:
                ResumeCertificateModel.ResultBean.CertificatesBean resumesBean= (ResumeCertificateModel.ResultBean.CertificatesBean) object2;
                //  updatetitlte(resumesBean.getTitle(),true,postion);

                Intent intent = new Intent(activity, CommonActivity.class);
                intent.putExtra(Constant.DESTINATION_FRAGMENT, Constant.GoTo.CREATE_RESUME_CERTIFICATE_EDIT);
                intent.putExtra(Constant.KEY_ID, resumeid);
                intent.putExtra(Constant.KEY_CERTIFICATE_ID, resumesBean.getCertificate_id());
    ...
    public void showDeleteDialog(final Context context, final int certificateid) {
        try {
            ...
            tvMsg.setText(R.string.MSG_DELETE_CONFIRMATION_CERT);
            ...
            progressDialog.findViewById(R.id.bCamera).setOnClickListener(v -> {
                progressDialog.dismiss();
                calldeletetitle(false, Constant.CREDIT_RESUME_DELETE_CERTIFICATE,certificateid);
    ...
    private void calldeletetitle(boolean showLoader,String url,int certificate_id) {
        try {
            ...
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    request.params.put(Constant.KEY_AUTH_TOKEN, SPref.getInstance().getToken(context));
                    request.params.put("certificate_id", certificate_id);
                    request.requestMethod = HttpPost.METHOD_NAME;
                    request.headres.put(Constant.KEY_COOKIE, getCookie());
