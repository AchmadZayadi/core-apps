...
public class AchivementsAdpterSub extends RecyclerView.Adapter<AchivementsAdpterSub.ContactHolder> {

    private final List<AchivementParentModel.ResultBean.CurricularsBean> list;
   ...
   /* @Override
    ...
    public AchivementsAdpterSub(List<AchivementParentModel.ResultBean.CurricularsBean> list, Context cntxt, OnUserClickedListener<Integer, Object> listenr) {
        this.list = list;
        this.listCat = null;
        this.context = cntxt;
    ...
    public void onBindViewHolder(@NonNull final ContactHolder holder, int position) {
        ...
        try {
            ...
            holder.titleview.setText(""+list.get(position).getCurricularname());
                    ...
                    popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                        ...
                        public boolean onMenuItemClick(MenuItem item) {
                            switch (item.getItemId()) {
                                case R.id.editmanue:
                                    //handle menu1 click
                                    listener.onItemClicked(Constant.Events.CLICKED_CUR_EDIT, list.get(position), list.get(position).getCurricular_id());
                                    break;
                                case R.id.deletemanue:
                                    //handle menu2 click
                                    listener.onItemClicked(Constant.Events.CLICKED_CUR_DELETE, list.get(position), list.get(position).getCurricular_id());
                                    break;
                            }
                            return false;
                        }
