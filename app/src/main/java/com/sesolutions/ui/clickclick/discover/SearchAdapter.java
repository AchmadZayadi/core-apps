...
public class SearchAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public SearchAdapter(List<VideoVo> list, Context cntxt, OnUserClickedListener<Integer, Object> listenr, OnLoadMoreListener loadListener) {
        this.list = list;
        this.context = cntxt;
            ...
            switch (list.get(position).getType()) {
                case VT_CATEGORY:
                    ...
                    if (((List) (List<VideoContent>) list.get(holder1.getAdapterPosition()).getValue()).size() > 0) {
                        ...
                        holder1.tvSeeAll.setOnClickListener(v -> listener.onItemClicked(Constant.Events.SEE_MORE, null, holder1.getAdapterPosition()));
                        holder1.rvChild.setOnClickListener(v -> listener.onItemClicked(Constant.Events.SEE_MORE, null, holder1.getAdapterPosition()));
                        //    holder.rvChild.setRecycledViewPool(viewPool);
                        final LinearLayoutManager layoutManager = new LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false);
                        holder1.rvChild.setLayoutManager(layoutManager);
                        holder1.adapter = new SearchActivityAdapter((List<Videos>) list.get(holder1.getAdapterPosition()).getValue(), context, listener);
    ...
    public static class CategoryHolder extends RecyclerView.ViewHolder {
        ...
        protected SearchActivityAdapter adapter;
