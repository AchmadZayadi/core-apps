...
public class DiscoverAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public DiscoverAdapter(List<VideoVo> list, Context cntxt, OnUserClickedListener<Integer, Object> listenr, OnLoadMoreListener loadListener) {
        this.list = list;
        this.context = cntxt;
        ...
        try {
            switch (list.get(position).getType()) {
                case VT_CATEGORY:
                    ...
                    if (((List) (List<VideoContent>) list.get(holder1.getAdapterPosition()).getValue()).size() > 0) {

                        themeManager.applyTheme((ViewGroup) holder1.itemView, context);

                        /*set child item list*/
                        holder1.llTop.setVisibility(View.VISIBLE);
                        holder1.rvChild.setHasFixedSize(true);
                        holder1.tvCategoryName.setText(list.get(position).getTitle());
                        holder1.tvSeeAll.setText("See All >");

                        GradientDrawable drawable = (GradientDrawable) holder1.tvSeeAll.getBackground();
                        drawable.setColor(menuTitleActiveColor);



                        holder1.tvSeeAll.setOnClickListener(v -> listener.onItemClicked(Constant.Events.MUSIC_CHANGED, null, holder1.getAdapterPosition()));
                        //    holder.rvChild.setRecycledViewPool(viewPool);
                        final LinearLayoutManager layoutManager = new LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false);
                        holder1.rvChild.setLayoutManager(layoutManager);
                        holder1.adapter = new DiscoverActivityAdapter((List<Videos>) list.get(holder1.getAdapterPosition()).getValue(), context, listener);
