...
public class ViewBusinessFragment extends CommentLikeHelper implements PopupMenu.OnMenuItemClickListener, View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, TabLayout.OnTabSelectedListener, OnUserClickedListener<Integer, Object> {
    ...
    public static ViewBusinessFragment newInstance(int groupId) {
        ViewBusinessFragment frag = new ViewBusinessFragment();
        frag.mBusinessId = groupId;
        return frag;
    }
            ...
            switch (activity.taskPerformed) {
                case Constant.TASK_IMAGE_UPLOAD:
                    if (activity.taskId == Constant.TASK_PHOTO_UPLOAD) {
                        result.getBusiness().getImages().setMain(Constant.BASE_URL + activity.stringValue);
                        updateProfilePhoto(Constant.BASE_URL + activity.stringValue);
                    } else if (activity.taskId == Constant.TASK_COVER_UPLOAD) {
                        result.getBusiness().getCoverImage().setMain(Constant.BASE_URL + activity.stringValue);
                ...
                case Constant.FormType.CREATE_ALBUM_OTHERS:
                    ...
                    map1.put(Constant.KEY_BUSINESS_ID, mBusinessId);
    ...
    private void initRelatedBusinessUI() {
        ...
        relatedList = new ArrayList<BusinessContent>();
        rvPhotos.setHasFixedSize(true);
        final LinearLayoutManager layoutManager = new LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false);
        rvPhotos.setLayoutManager(layoutManager);
        adapterRelated = new SuggestionBusinessAdapter(relatedList, context, this, true);
        ...
        try {
            if (isNetworkAvailable(context)) {
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_VIEW_BUSINESS);
                            ...
                            try {
                                ...
                                if (response != null) {
                                    ...
                                    if (TextUtils.isEmpty(err.getError())) {
                                        ...
                                        BusinessResponse commonResponse = new Gson().fromJson(response, BusinessResponse.class);
   ...
   /* @Override
    ...
    private void setUpperUIData() {
        ...
        if (result.getBusiness() != null) {
            ...
            BusinessContent resp = result.getBusiness();
            ...
            for (Options opt : list) {
                ...
                switch (opt.getName()) {
                    case Constant.TabOption.INFO:
                        ...
                        adapter.addFragment(BusinessInfoFragment.newInstance(mBusinessId), opt.getLabel());
                        break;
                    ...
                    case Constant.TabOption.POSTS:
                       /* Bundle bundle = new Bundle();
                        ...
                        adapter.addFragment(FeedFragment.newInstance(mBusinessId, Constant.ResourceType.BUSINESS), opt.getLabel());
                        break;
                    ...
                    case Constant.TabOption.MAP:
                        ...
                        bundle.putInt(Constant.KEY_RESOURCE_ID, mBusinessId);
                        map = new HashMap<>();
                        map.put(Constant.KEY_BUSINESS_ID, mBusinessId);
                    ...
                    case Constant.TabOption.SERVICES:
                        ...
                        bundle.putInt(Constant.KEY_RESOURCE_ID, mBusinessId);// mPageId);
                        map = new HashMap<>();
                        map.put(Constant.KEY_BUSINESS_ID, mBusinessId);
                        bundle.putSerializable(Constant.POST_REQUEST, map);
                        bundle.putString(Constant.KEY_RESOURCES_TYPE, Constant.ResourceType.BUSINESS);
                    ...
                    case "associatebusinesses":
                        adapter.addFragment(BusinessFragment.newInstance(BusinessFragment.TYPE_ASSOCIATE, mBusinessId), opt.getLabel());
                        break;
                    case "claim":
                        map = new HashMap<>();
                        map.put(Constant.KEY_BUSINESS_ID, mBusinessId);
                    ...
                    case Constant.TabOption.VIDEO:
                        adapter.addFragment(PageVideoFragment.newInstance(opt.getName(), Constant.ResourceType.BUSINESS, mBusinessId, this), opt.getLabel());
                        break;
                    ...
                    case Constant.TabOption.MEMBERS:
                        ...
                        map.put(Constant.KEY_BUSINESS_ID, mBusinessId);
                        bundle.putSerializable(Constant.POST_REQUEST, map);
                        adapter.addFragment(BusinessMemberFragment.newInstance(bundle), opt.getLabel());
                        break;
                    default:
            ...
            public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
                try {
                    ...
                    if (scrollRange + verticalOffset == 0) {
                        collapsingToolbar.setTitle(result.getBusiness().getTitle());
                        isShow = true;
                    } else if (isShow) {
    ...
    private void addUpperTabItems() {
        ...
        //add favourite item
        if (result.getBusiness().canLike()) {

            ((TextView) view1.findViewById(R.id.tvOptionText)).setText(result.getBusiness().isContentLike() ? R.string.TXT_UNLIKE : R.string.TXT_LIKE);
            ((ImageView) view1.findViewById(R.id.ivOptionImage)).setImageDrawable(ContextCompat.getDrawable(context, R.drawable.like));
            ((ImageView) view1.findViewById(R.id.ivOptionImage)).setColorFilter(result.getBusiness().isContentLike() ? Color.parseColor(Constant.colorPrimary) : color);
            ...
            view1.setOnClickListener(v -> {
                ...
                if (result.getBusiness().hasToChangeFollowLike()) {
                    callLikeApi(REQ_FOLLOW, view2, Constant.URL_BUSINESS_FOLLOW, false);
                }
                callLikeApi(REQ_LIKE, view1, Constant.URL_BUSINESS_LIKE, true);
            });
            llTabOptions.addView(view1);
        }

        //add favourite item
        if (result.getBusiness().canFavourite()) {
            ...
            ((ImageView) view3.findViewById(R.id.ivOptionImage)).setColorFilter(result.getBusiness().isContentFavourite() ? Color.parseColor(Constant.red) : color);
            ((TextView) view3.findViewById(R.id.tvOptionText)).setTextColor(color);
            view3.setOnClickListener(v -> callLikeApi(REQ_FAVORITE, view3, Constant.URL_BUSINESS_FAVORITE, true));
            llTabOptions.addView(view3);
        }

        //add Follow item
        if (mUserId != result.getBusiness().getOwner_id() && result.getBusiness().canFollow()) { //don't show follow button to Business Owner

            ((TextView) view2.findViewById(R.id.tvOptionText)).setText(result.getBusiness().isContentFollow() ? R.string.unfollow : R.string.follow);
            ((ImageView) view2.findViewById(R.id.ivOptionImage)).setImageDrawable(ContextCompat.getDrawable(context, result.getBusiness().isContentFollow() ? R.drawable.unfollow : R.drawable.follow));
            ...
            view2.setOnClickListener(v -> {
                ...
                //if likeFollow setting enabled then also call like api
                if (result.getBusiness().hasToChangeFollowLike()) {
                    callLikeApi(REQ_LIKE, view1, Constant.URL_BUSINESS_LIKE, false);
                }
                callLikeApi(REQ_FOLLOW, view2, Constant.URL_BUSINESS_FOLLOW, true);
            });
            llTabOptions.addView(view2);
        }

        //add more item
        if (null != result.getBusiness().getButtons()) {
            ...
            view.setOnClickListener(v -> showPopup(result.getBusiness().getButtons(), view, 2000));
    ...
    private void updateRelatedBusinessAdapter() {
        try {
            relatedList.clear();
            if (result.getRelatedBusinesses() != null && result.getRelatedBusinesses().size() > 0) {
                v.findViewById(R.id.rlRecent).setVisibility(View.VISIBLE);
                relatedList.addAll(result.getRelatedBusinesses());
            } else {
            ...
            for (final Options opt : result.getAbout()) {
                switch (opt.getName()) {
                    ...
                    case Constant.OptionType.TAG:
                        view = getLayoutInflater().inflate(R.layout.layout_text_image_horizontal, (ViewGroup) llAbout, false);
                        ((TextView) view.findViewById(R.id.tvOptionText)).setText(SpanUtil.getHashTags(result.getBusiness().getTag(), this));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        try {
            switch (item.getItemId()) {
                case R.id.share:
                    showShareDialog(result.getBusiness().getShare());
                    break;
                case R.id.option:
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        try {
            ...
            if (null != result && null != result.getBusiness() && result.getBusiness().getShare() != null) {

                menu.add(Menu.NONE, R.id.share, Menu.FIRST, result.getBusiness().getShare().getLabel())
    ...
    public boolean onMenuItemClick(MenuItem item) {
        try {
            ...
            if (itemId > 2000) {
                itemId = itemId - 2000;
                opt = result.getBusiness().getButtons().get(itemId - 1);
            } else if (itemId > 1000) {
                itemId = itemId - 1000;
                opt = result.getBusiness().getUpdateCoverPhoto().get(itemId - 1);
                isCover = true;
            } else if (itemId > 100) {
                itemId = itemId - 100;
                opt = result.getBusiness().getUpdateProfilePhoto().get(itemId - 1);
            } else {
                itemId = itemId - 10;
            ...
            switch (opt.getName()) {
                case Constant.OptionType.EDIT:
                    ...
                    map.put(Constant.KEY_BUSINESS_ID, mBusinessId);
                    fragmentManager.beginTransaction().replace(R.id.container, CreateEditBusinessFragment.newInstance(Constant.FormType.EDIT_BUSINESS, map, Constant.URL_BUSINESS_EDIT, null)).addToBackStack(null).commit();
                    break;
                ...
                case Constant.OptionType.REPORT_SMOOTHBOX:
                    goToReportFragment(Constant.ResourceType.BUSINESS + "_" + mBusinessId);
                    break;
                ...
                case Constant.OptionType.INVITE:
                    ...
                    InviteDialogFragment.newInstance(this, map, Constant.URL_BUSINESS_INVITE).show(fragmentManager, "social");
                    break;


                case "likeasyourbusiness":
                    // BusinessLike
                    if (isNetworkAvailable(context)) {
                        showBaseLoader(false);
                        map = new HashMap<>();
                        map.put(Constant.KEY_ID, mBusinessId);
                ...
                case Constant.OptionType.view_profile_photo:
                    //  goToGalleryFragment(result.getBusiness().getPhotoId(), resourceType, result.getProfile().getUserPhoto());
                    break;
                case Constant.OptionType.ALBUM:
                    ...
                    map.put(Constant.KEY_ID, mBusinessId);
                    map.put(Constant.KEY_TYPE, Constant.ResourceType.BUSINESS);
                    openSelectAlbumFragment(isCover ? Constant.URL_UPLOAD_BUSINESS_COVER : Constant.URL_UPLOAD_BUSINESS_PHOTO, map);
                    break;
                ...
                case Constant.OptionType.UPLOAD:
                    ...
                    if (isCover) {
                        ...
                        goToUploadAlbumImage(Constant.URL_UPLOAD_BUSINESS_COVER, result.getBusiness().getCoverImageUrl(), opt.getLabel(), map);
                    } else {
                        map.put(Constant.KEY_TYPE, Constant.TASK_PHOTO_UPLOAD);
                        goToUploadAlbumImage(Constant.URL_UPLOAD_BUSINESS_PHOTO, result.getBusiness().getMainImageUrl(), opt.getLabel(), map);
                    }
                    break;
                case Constant.OptionType.view_cover_photo:
                    // goToGalleryFragment(result.getBusiness().getCoverImageUrl(), resourceType, result.getBusiness().getCoverImageUrl());
                    break;
    ...
    public void showImageRemoveDialog(boolean isCover) {
        try {
            final String url = isCover ? Constant.URL_REMOVE_BUSINESS_COVER : Constant.URL_REMOVE_BUSINESS_PHOTO;
            ...
            progressDialog.findViewById(R.id.bCamera).setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    map.put(Constant.KEY_BUSINESS_ID, mBusinessId);
                    map.put(Constant.KEY_TYPE, Constant.ResourceType.BUSINESS);
                    new ApiController(url, map, context, ViewBusinessFragment.this, Constant.Events.REMOVE_PHOTO).execute();
                }
            });
    ...
    public void onClick(View view) {
        try {
            switch (view.getId()) {
                ...
                case R.id.bContact:
                    super.openBusinessContactForm(result.getBusiness().getOwner_id());
                    break;

                case R.id.ivCamera2:
                    showPopup(result.getBusiness().getUpdateProfilePhoto(), v.findViewById(R.id.ivCamera2), 100);
                    break;
                case R.id.ivCamera:
                    if (null != result.getBusiness().getUpdateCoverPhoto())
                        showPopup(result.getBusiness().getUpdateCoverPhoto(), v.findViewById(R.id.ivCamera), 1000);
                    break;
               /* case R.id.like_heart:
                    callReactionApi(AppConstantSes.URL_LIKE + mBusinessId, view);
                    resp.toggleLike();
                ...
                case R.id.appreciate_heart:
                    callReactionApi(AppConstantSes.URL_APPRECIATE + mBusinessId, v);
                    resp.toggleFollow();
    ...
    public void showDeleteDialog() {
        try {
            ...
            progressDialog.findViewById(R.id.bCamera).setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    progressDialog.dismiss();
                    callDeleteApi(REQ_DELETE, Constant.URL_DELETE_BUSINESS);
                }
            });
    ...
    private void callFragment(Object value) {
        ...
        switch (opt.getName()){
            case Constant.TabOption.INFO:
                ...
                intent2.putExtra(Constant.DESTINATION_FRAGMENT, Constant.GoTo.PROFILE_BUSINUSS_INFO);
                intent2.putExtra(Constant.KEY_ID, mBusinessId);
            ...
            case Constant.TabOption.POLL:
                bundle = new Bundle();
                map = new HashMap<>();
                map.put(Constant.KEY_PAGE_ID, mBusinessId);
                map.put(Constant.KEY_BUSINESS_ID, mBusinessId);
                bundle.putSerializable(Constant.POST_REQUEST, map);
        ...
        try {
            ...
            switch (object1) {
                ...
                case Constant.Events.LIKE_AS_PAGE:
                    hideBaseLoader();
                    handleLikeAsBusinessResponse(object2, Constant.URL_LIKE_AS_BUSINESS);
                    break;

                case Constant.Events.UNLIKE_AS_PAGE:
                    hideBaseLoader();
                    handleLikeAsBusinessResponse(object2, Constant.URL_UNLIKE_AS_BUSINESS);
                    break;
        ...
        try {
            if (null != response) {
                ...
                if (TextUtils.isEmpty(err.getErrorMessage())) {
                    ...
                    if (res != null) {
                        res.setId(mBusinessId);
                        PageLikeDialogFragment.newInstance(res, url, this, Constant.ResourceType.BUSINESS).show(fragmentManager, Constant.TITLE_PRIVACY);
                    } else {
                        somethingWrongMsg(v);
                    }
                } else {
    ...
    private void callLikeApi(final int REQ_CODE, final View view, String url, boolean showAnimation) {
        ...
        try {
            if (isNetworkAvailable(context)) {
                updateItemLikeFavorite(REQ_CODE, view, result.getBusiness(), showAnimation);
                try {

                    HttpRequestVO request = new HttpRequestVO(url);

                    request.params.put(Constant.KEY_ID, mBusinessId);
                    request.params.put(Constant.KEY_TYPE, Constant.ResourceType.BUSINESS);
                    request.headres.put(Constant.KEY_COOKIE, getCookie());
                    request.params.put(Constant.KEY_AUTH_TOKEN, SPref.getInstance().getToken(context));
                    request.requestMethod = HttpPost.METHOD_NAME;
                                ...
                                if (response != null) {
                                    ...
                                    if (TextUtils.isEmpty(err.getError())) {
                                        ...
                                        if (REQ_CODE > REQ_DELETE) {
                                            ...
                                            result.getBusiness().setButtons(opt);
                                        ...
                                        updateItemLikeFavorite(REQ_CODE, view, result.getBusiness(), false);
                                        Util.showSnackbar(v, err.getErrorMessage());
                                    }
                                }
    ...
    public void updateItemLikeFavorite(int REQ_CODE, View view, BusinessContent vo, boolean showAnimation) {

        if (REQ_CODE == REQ_LIKE) {
            vo.setContentLike(!vo.isContentLike());
            if (showAnimation)
                ((SmallBangView) view.findViewById(R.id.vBang)).likeAnimation();
            ((TextView) view.findViewById(R.id.tvOptionText)).setText(result.getBusiness().isContentLike() ? R.string.TXT_UNLIKE : R.string.TXT_LIKE);
            ...
            if (showAnimation)
                ((SmallBangView) view.findViewById(R.id.vBang)).likeAnimation();
            ((TextView) view.findViewById(R.id.tvOptionText)).setText(result.getBusiness().isContentFollow() ? R.string.unfollow : R.string.follow);
        }
                            ...
                            if (response != null) {
                                ...
                                if (err.isSuccess()) {
                                    ...
                                    if (resp != null && resp.getResult() != null && resp.getResult().getCategory() != null) {
                                        Map<String, Object> map = new HashMap<>();
                                        map.put(Constant.KEY_PARENT_ID, mBusinessId);
                                        ...
                                        if (vo != null && vo.getResult() != null && vo.getResult().getFormfields() != null) {
                                            Map<String, Object> map = new HashMap<>();
                                            map.put(Constant.KEY_PARENT_ID, mBusinessId);
                                            openBusinessCreateForm(vo.getResult(), map);
                                        }
                                    }
                                } else {
                                    Util.showSnackbar(v, err.getErrorMessage());
