...
public class NotificationFragment extends BaseFragment implements View.OnClickListener, OnLoadMoreListener, OnUserClickedListener<Integer, String>, SwipeRefreshLayout.OnRefreshListener {
    ...
    public static NotificationFragment newInstance(OnUserClickedListener<Integer, Object> parent) {
    ...
    private void setRecyclerView() {
        try {
            ...
            adapter = new NotificationAdapter(friendList, context, this, this);
        ...
        try {
            ...
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URl_NOTIFICATION);
                    ...
                    Handler.Callback callback = new Handler.Callback() {
                        ...
                        public boolean handleMessage(Message msg) {
                            ...
                            try {
                                ...
                                if (response != null) {
                                    ((MainActivity) activity).dashboardFragment.isNotificationContentLoaded = true;
    ...
    public boolean onItemClicked(Integer id, String type, int postion) {
        try {
            ...
            if (vo.getObjectType().equals("activity_action")) {
                if (SPref.getInstance().getDefaultInfo(getContext(), Constant.KEY_APPDEFAULT_DATA).getResult().isIs_core_activity()) {
                    performClick(Constant.VALUE_RESOURCES_TYPE2,
                            vo.getObjectId(),
                            vo.getHref(),
                            vo.getIscommentlike());
                } else {
                    performClick(Constant.VALUE_RESOURCES_TYPE,
                            vo.getObjectId(),
                            vo.getHref(),
                            vo.getIscommentlike());
                }
            } else if (vo.getObjectType().equals(Constant.ResourceType.LIVE_HOST)) {
                performClick(Constant.ResourceType.LIVE_STREAMING,
                        vo.getHostId(),
                        vo.getHref(),
                        vo.getIscommentlike(),
                        vo.getActivityId());
            } else if (vo.getObjectType().equals(Constant.MESSAGE_CONVERSATION)) {
                Intent intent = new Intent(activity, ChatActivity.class);
                MessageInbox messageInbox = new MessageInbox();
                messageInbox.setConversationId(vo.getObjectId());
                messageInbox.setSender(vo.getUserName());
                intent.putExtra(Constant.KEY_DATA, (Serializable) messageInbox);
                startActivity(intent);
            } else {
                performClick(vo.getObjectType()
                        , (vo.getObjectType().equals(Constant.ResourceType.USER)) ? vo.getSubjectId() :
                                vo.getObjectId(),
                        vo.getHref(),
                        vo.getIscommentlike());
            }
