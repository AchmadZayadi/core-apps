...
public class ViewPrayerFragment extends CommentLikeHelper implements View.OnClickListener, PopupMenu.OnMenuItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle saveInstanceState) {
        ...
        try {
            ...
            init();
            callMusicAlbumApi(1);
            callBottomCommentLikeApi(albumId, Constant.ResourceType.PRAYER, Constant.URL_VIEW_COMMENT_LIKE);
            if (openComment) {
                //change value of openComment otherwise it prevents coming back from next screen
                openComment = false;
                goToCommentFragment(albumId, Constant.ResourceType.PRAYER);
            }
        } catch (Exception e) {
            CustomLog.e(e);
        }
               ...
               /* if (scrollRange == -1) {
                ...
                if (/*scrollRange +*/ verticalOffset == 0) {
                    if (album != null) {
                        ...
                        collapsingToolbar.setTitle(album.getPrayerTitle());
                    }
                    //  isShow = true;
                } else {
    ...
    private void updateUpperLayout() {
        try {
            ...
            if (!TextUtils.isEmpty(album.getPrayerTitle())) {
                tvQuoteBy.setText(album.getPrayerTitle());
            } else {
  ...
  /*  private void callRemoveImageApi(String url) {
        ...
        try {
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_VIEW_PRAYER);
                    request.params.put(Constant.KEY_LIMIT, Constant.RECYCLE_ITEM_THRESHOLD);
                    request.params.put(Constant.KEY_PRAYER_ID, albumId);
    ...
    public static ViewPrayerFragment newInstance(int albumId, boolean openComment) {
        ViewPrayerFragment frag = new ViewPrayerFragment();
        frag.albumId = albumId;
        frag.openComment = openComment;
        return frag;
    }

    public static ViewPrayerFragment newInstance(int albumId) {
        return ViewPrayerFragment.newInstance(albumId, false);
    }
        ...
        try {
            if (isNetworkAvailable(context)) {
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_DELETE_PRAYER);
                    request.params.put(Constant.KEY_PRAYER_ID, albumId);
        ...
        try {
            ...
            switch (opt.getName()) {
                case Constant.OptionType.EDIT:
                    goToFormFragment(album.getPrayerId());
                    break;
                case Constant.OptionType.DELETE:
                    showDeleteDialog();
                    break;
                case Constant.OptionType.REPORT:
                    goToReportFragment(Constant.ModuleName.PRAYER + "_" + album.getPrayerId());
                    break;
    ...
    public void goToFormFragment(int blogId) {
        ...
        map.put(Constant.KEY_PRAYER_ID, blogId);
        // map.put(Constant.KEY_GET_FORM, 1);
        fragmentManager.beginTransaction().replace(R.id.container, FormFragment.newInstance(Constant.FormType.EDIT_PRAYER, map, Constant.URL_EDIT_PRAYER)).addToBackStack(null).commit();
    }
    ...
    public void showDeleteDialog() {
        try {
            ...
            tvMsg.setText(Constant.MSG_DELETE_CONFIRMATION_PRAYER);
    ...
    public void onStart() {
        ...
        if (activity.taskPerformed == Constant.FormType.EDIT_PRAYER) {
            activity.taskPerformed = 0;
            onRefresh();
            //     callMusicAlbumApi(1);
        }
