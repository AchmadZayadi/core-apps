...
public class PrayerParentFragment extends BaseFragment implements View.OnClickListener, OnUserClickedListener<Integer, Object> {
    ...
    private void setupViewPager() {
        ...
        adapter.addFragment(BrowsePrayerFragment.newInstance(this, 0), Constant.TAB_TITLE_PRAYERS_1);
        ...
        if (SPref.getInstance().isLoggedIn(context)) {
            ...
            adapter.addFragment(ManagePrayerFragment.newInstance(this, 0), Constant.TAB_TITLE_PRAYERS_2);
            adapter.addFragment(
                    CreateQuoteFragment.newinstance(Constant.FormType.CREATE_PRAYER, Constant.URL_CREATE_PRAYER, this), Constant.TAB_TITLE_PRAYERS_3);
        }
    ...
    private void refreshScreenByPosition(int position) {
        ...
        try {
            switch (position) {
                case 0:
                    if (!isQuoteLoaded)
                        ((BrowsePrayerFragment) (adapter.getItem(position))).onRefresh();
                    break;

                case 1:
                    if (!isMyQuoteLoaded)
                        ((ManagePrayerFragment) adapter.getItem(position)).onRefresh();
                    break;
...
*/
    ...
    private void loadFragmentIfNotLoaded(int position) {
        ...
        try {
            switch (position) {
                case 0:
                    if (!isQuoteLoaded)
                        ((BrowsePrayerFragment) (adapter.getItem(position))).initScreenData();
                    break;

                case 1:
                    if (!isMyQuoteLoaded)
                        ((ManagePrayerFragment) adapter.getItem(position)).initScreenData();
                    break;
   ...
   /* private void goToFormFragment() {
    ...
    private void goToSearchFragment() {
        fragmentManager.beginTransaction().replace(R.id.container, new SearchPrayerFragment()).addToBackStack(null).commit();
    }
