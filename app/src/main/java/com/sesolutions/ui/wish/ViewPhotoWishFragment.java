...
public class ViewPhotoWishFragment extends CommentLikeHelper implements View.OnClickListener, PopupMenu.OnMenuItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle saveInstanceState) {
        ...
        try {
            ...
            init();
            callMusicAlbumApi(1);
            callBottomCommentLikeApi(albumId, Constant.ResourceType.WISH, Constant.URL_VIEW_COMMENT_LIKE);
            if (openComment) {
                //change value of openComment otherwise it prevents coming back from next screen
                openComment = false;
                goToCommentFragment(albumId, Constant.ResourceType.WISH);
            }
        } catch (Exception e) {
            CustomLog.e(e);
        }
    ...
    private void updateUpperLayout() {
        try {
            ...
            collapsingToolbar.setTitle(album.getWishTitle());
            //  setRatingStars();
            ...
            if (!TextUtils.isEmpty(album.getWishTitle())) {
                tvQuoteBy.setText(album.getWishTitle());
            } else {
  ...
  /*  private void callRemoveImageApi(String url) {
        ...
        try {
            ...
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_VIEW_WISH);
                    request.params.put(Constant.KEY_LIMIT, Constant.RECYCLE_ITEM_THRESHOLD);
                    request.params.put(Constant.KEY_WISH_ID, albumId);
    ...
    public static ViewPhotoWishFragment newInstance(int albumId, boolean openComment) {
        ViewPhotoWishFragment frag = new ViewPhotoWishFragment();
        frag.albumId = albumId;
        frag.openComment = openComment;
        return frag;
    }

    public static ViewPhotoWishFragment newInstance(int albumId) {
        return ViewPhotoWishFragment.newInstance(albumId, false);
    }
        ...
        try {
            if (isNetworkAvailable(context)) {
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_DELETE_WISH);
                    request.params.put(Constant.KEY_WISH_ID, albumId);
        ...
        try {
            ...
            switch (opt.getName()) {
                case Constant.OptionType.EDIT:
                    goToFormFragment(album.getWishId());
                    break;
                case Constant.OptionType.DELETE:
                    showDeleteDialog();
                    break;
                case Constant.OptionType.REPORT:
                    goToReportFragment(Constant.ResourceType.WISH + "_" + album.getWishId());
                    break;
    ...
    public void goToFormFragment(int blogId) {
        ...
        map.put(Constant.KEY_WISH_ID, blogId);
        // map.put(Constant.KEY_GET_FORM, 1);
        fragmentManager.beginTransaction().replace(R.id.container, FormFragment.newInstance(Constant.FormType.EDIT_WISH, map, Constant.URL_EDIT_WISH)).addToBackStack(null).commit();
    }
    ...
    public void showDeleteDialog() {
        try {
            ...
            tvMsg.setText(getString(R.string.MSG_DELETE_CONFIRMATION_WISH));
    ...
    public void onStart() {
        ...
        if (activity.taskPerformed == Constant.FormType.EDIT_WISH) {
            activity.taskPerformed = 0;
            onRefresh();
            //     callMusicAlbumApi(1);
        }
