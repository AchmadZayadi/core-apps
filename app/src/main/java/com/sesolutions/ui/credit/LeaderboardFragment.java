...
public class LeaderboardFragment extends BaseFragment implements View.OnClickListener, OnLoadMoreListener, OnUserClickedListener<Integer, Object>, SwipeRefreshLayout.OnRefreshListener {

    private static final int REQ_LOAD_MORE = 100;
    public View v;
    public List<LeaderBoard> albumsList;
    ...
    public static LeaderboardFragment newInstance(String selectedScreen, OnUserClickedListener<Integer, Object> listener) {
    ...
    public void setRecyclerView() {
        try {
            ...
            adapter = new LeaderboardAdapter(albumsList, context, this, this);
        ...
        try {
            ...
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(URL.CREDIT_LEADERBOARD);
                                ...
                                if (response != null) {
                                    ...
                                    if (TextUtils.isEmpty(resp.getError())) {
                                        ...
                                        if (null != result.getItems())
                                            albumsList.addAll(result.getItems());
                                        ...
                                        if (null != resp.getResult().getMember()) {
                                         /*   Notifications vo = albumsList.get(position);
                                            ...
                                            if (url.contains("follow")) {
                                                albumsList.get(position).getUser().setFollow(resp.getResult().getMember().getFollow());
                                            } else {
                                                albumsList.get(position).getUser().setMembership(resp.getResult().getMember().getMembership());
                                            }
        ...
        try {
            ...
            switch (object1) {
                case Constant.Events.MEMBER_ADD:
                    String type = albumsList.get(postion).getUser().getMembership().getAction();
                    ...
                    callMemberStatusApi(url, postion, albumsList.get(postion).getUser().getUserId());
                    break;

                case Constant.Events.MEMBER_FOLLOW:
                    //  type = albumsList.get(postion).getFollow().getAction();
                    callMemberStatusApi(Constant.URL_FOLLOW_MEMBER, postion, albumsList.get(postion).getUser().getUserId());
                    break;

                case Constant.Events.CLICKED_HEADER_IMAGE:

                    goToProfileFragment(albumsList.get(postion).getUser().getUserId(), (LeaderboardAdapter.ContactHolder) object2, postion);
                    break;
            }
        } catch (Exception e) {
            CustomLog.e(e);
    ...
    public void goToProfileFragment(int userId, LeaderboardAdapter.ContactHolder holder, int position) {
        try {
            String transitionName = albumsList.get(position).getUser().getTitle();
            ...
            bundle.putString(Constant.Trans.IMAGE_URL, albumsList.get(position).getUser().getUserImage());
