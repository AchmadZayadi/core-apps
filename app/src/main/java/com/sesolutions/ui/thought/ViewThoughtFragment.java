...
public class ViewThoughtFragment extends CommentLikeHelper implements View.OnClickListener, PopupMenu.OnMenuItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle saveInstanceState) {
        ...
        try {
            ...
            init();
            callMusicAlbumApi(1);
            callBottomCommentLikeApi(albumId, Constant.ResourceType.THOUGHT, Constant.URL_VIEW_COMMENT_LIKE);
            if (openComment) {
                //change value of openComment otherwise it prevents coming back from next screen
                openComment = false;
                goToCommentFragment(albumId, Constant.ResourceType.THOUGHT);
            }
        } catch (Exception e) {
            CustomLog.e(e);
        }
    ...
    private void updateUpperLayout() {
        try {
            ...
            collapsingToolbar.setTitle(album.getThoughtTitle());
            //  setRatingStars();
            ...
            if (!TextUtils.isEmpty(album.getThoughtTitle())) {
                tvQuoteBy.setText(album.getThoughtTitle());
            } else {
  ...
  /*  private void callRemoveImageApi(String url) {
        ...
        try {
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_VIEW_THOUGHT);
    ...
    public static ViewThoughtFragment newInstance(int albumId, boolean openComment) {
        ViewThoughtFragment frag = new ViewThoughtFragment();
        frag.albumId = albumId;
        frag.openComment = openComment;
        return frag;
    }

    public static ViewThoughtFragment newInstance(int albumId) {
        return ViewThoughtFragment.newInstance(albumId, false);
    }
        ...
        try {
            ...
            switch (opt.getName()) {
                case Constant.OptionType.EDIT:
                    goToFormFragment(album.getThoughtId());
                    break;
                case Constant.OptionType.DELETE:
                    showDeleteDialog();
                    break;
                case Constant.OptionType.REPORT:
                    goToReportFragment(Constant.ModuleName.THOUGHT + "_" + album.getThoughtId());
                    break;
    ...
    public void onStart() {
        ...
        if (activity.taskPerformed == Constant.FormType.EDIT_THOUGHT) {
            activity.taskPerformed = 0;
            onRefresh();
            //     callMusicAlbumApi(1);
        }
