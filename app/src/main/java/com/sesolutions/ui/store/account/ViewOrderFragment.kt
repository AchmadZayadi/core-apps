...
class ViewOrderFragment : BaseFragment(), androidx.swiperefreshlayout.widget.SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
    ...
    private lateinit var productList: MutableList<ViewOrder.Result.Products.ProductsData>
    private lateinit var orderInfoList: MutableList<ViewOrder.Result.Otherinfo>
    lateinit var adapter: ViewOrderAdapter
    lateinit var orderInfoAdapter: OrderInfoAdapter
    private var result: ViewOrder.Result? = null


    companion object {

        @JvmStatic
        fun newInstance(id: Int): ViewOrderFragment {
            val frag = ViewOrderFragment()
            frag.orderId = id
            return frag
        }
    }
    ...
    fun setRecyclerView() {
        try {
            ...
            adapter = ViewOrderAdapter(productList, context)
//            adapter.setType(selectedScreen)
            ...
            orderInfoAdapter = OrderInfoAdapter(orderInfoList)
    ...
    private fun callViewOrderApi(req: Int) {
        ...
        if (isNetworkAvailable(context)) {
            ...
            try {
                ...
                val request = HttpRequestVO(Constant.URL_VIEW_ORDERS) //url will change according to screenType
                    ...
                    try {
                        ...
                        if (response != null) {
                            ...
                            if (TextUtils.isEmpty(err.error)) {
                                ...
                                val resp = Gson().fromJson(response, ViewOrder::class.java)
                                ...
                                if (result!!.products.productsData.isNotEmpty())
                                    productList.addAll(result!!.products.productsData)
    ...
    private fun setUpperUIData() {
        ...
        v!!.findViewById<TextView>(R.id.tvBillingAddress).text = result!!.billing[0].name + "\n" + result!!.billing[0].address + " " + result!!.billing[0].city + " " + result!!.billing[0].phonecode + ",\n" + result!!.billing[0].stateName + " " + result!!.billing[0].billingName
        v!!.findViewById<TextView>(R.id.tvBillingPhone).text = result!!.billing[0].phoneNumber
        v!!.findViewById<TextView>(R.id.tvBillingEmail).text = result!!.billing[0].email
        ...
        v!!.findViewById<TextView>(R.id.tvorderId).text = result!!.products.productsData[0].parentOrderId.toString()
        ...
        v!!.findViewById<TextView>(R.id.tvLabelOrder).text = result!!.products.title
