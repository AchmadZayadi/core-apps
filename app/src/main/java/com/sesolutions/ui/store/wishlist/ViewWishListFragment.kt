...
class ViewWishListFragment : CommentLikeHelper(), OnLoadMoreListener, androidx.swiperefreshlayout.widget.SwipeRefreshLayout.OnRefreshListener, View.OnClickListener, PopupMenu.OnMenuItemClickListener {
    ...
    private lateinit var wishlist: MutableList<StoreVo>
    lateinit var adapter: ProductAdapter
    private var result: ProductResponse.Result? = null
    ...
    companion object {
        ...
        fun newInstance(parent: OnUserClickedListener<Int, Any>?, loggedInId: Int, categoryId: Int): ViewWishListFragment {
            val frag = ViewWishListFragment()
            frag.parent = parent
        ...
        @JvmStatic
        fun newInstance(TYPE: String, id: Int): ViewWishListFragment {
            val frag = ViewWishListFragment()
...
//        optionbar = v.findViewById(R.id.optionsbar)
    ...
    fun setRecyclerView() {
        try {
            wishlist = ArrayList<StoreVo>()
            recyclerView.setHasFixedSize(true)
            recyclerView.layoutManager = androidx.recyclerview.widget.GridLayoutManager(context, 2, androidx.recyclerview.widget.GridLayoutManager.VERTICAL, false)
            (recyclerView.itemAnimator as androidx.recyclerview.widget.SimpleItemAnimator).supportsChangeAnimations = false
            adapter = ProductAdapter(wishlist, context, this, this)
    ...
    private fun callProductApi(req: Int) {
        ...
        if (isNetworkAvailable(context)) {
            ...
            try {
                ...
                val request = HttpRequestVO(Constant.URL_VIEW_WISHLIST) //url will change according to screenType
                ...
                val callback = Handler.Callback { msg ->
                    ...
                    try {
                        ...
                        if (TextUtils.isEmpty(err.error)) {
                            ...
                            val resp = Gson().fromJson(response, ProductResponse::class.java)
...
//                            setUpperUIData()
                            ...
                            if (null != result?.wishlist) {
                                ...
                                for (vo in result?.wishlist!!.products!!) {
                                    wishlist.add(StoreVo(selectedScreen, vo))
                                }
                            }
...
//                    showBaseLoader(false)
                    ...
                    ApiController(Constant.URL_DELETE_WISHLIST, map, context, this@ViewWishListFragment, -3).execute()
                } else {
                    notInternetMsg(v)
                }
            }
