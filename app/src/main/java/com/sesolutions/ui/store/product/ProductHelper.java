...
public class ProductHelper<T extends RecyclerView.Adapter> extends CommentLikeHelper implements View.OnClickListener {
    ...
    public ProductResponse.Result result;
...
//                    if (videoList.get(postion).getItem().hasToChangeFollowLike()) {
                    if (videoList.get(postion).getItem().hasToChangeFollowLike()) {
                        callLikeApi(REQ_FOLLOW, postion, Constant.URL_PRODUCT_FOLLOW, -2);
                    }
                    callLikeApi(REQ_LIKE, postion, Constant.URL_PRODUCT_LIKE, -1);
                    break;
                ...
                case Constant.Events.MUSIC_ADD:
                    ...
                    if (videoList.get(postion).getItem().hasToChangeFollowLike()) {
                        callLikeApi(REQ_LIKE, postion, Constant.URL_PRODUCT_LIKE, -2);
                    }
                    callLikeApi(REQ_FOLLOW, postion, Constant.URL_PRODUCT_FOLLOW, -1);
                    break;
                case Constant.Events.MUSIC_MAIN:
//                    openViewPageFragment(videoList.get(postion).getItem().getStoreId());
                    StoreUtil.openViewProductFragment(fragmentManager, videoList.get(postion).getItem().getProductId());
                    break;
                case Constant.Events.ADD_TO_CART:
                    callAddToCartApi(videoList.get(postion).getItem().getProductId());
                    break;
                ...
                case Constant.Events.CATEGORY:
                    if (categoryId != postion) { //do not open same category again
                        StoreUtil.openViewProductCategoryFragment(fragmentManager, postion, "" + screenType);
                    }
//                        openViewPageCategoryFragment(postion, "" + screenType);
                    break;
                case Constant.Events.OPEN_WISHLIST:
                    StoreUtil.openViewWishlistFragmnet(fragmentManager, (String) screenType, videoList.get(postion).getItem().getWishListId());
                    break;
...
//                                        }
    ...
    private void showJoinLeaveDialog(final int optionPosition, final int position) {
        ...
        switch (opt.getName()) {
            ...
            case Constant.OptionType.CANCEL:
                ...
                url[0] = Constant.URL_PRODUCT_CANCEL_MEMBER;
                req[0] = REQ_CANCEL;
                break;
            ...
            switch (opt.getName()) {
              ...
              /*  case Constant.OptionType.SHARE:
                ...
                case Constant.OptionType.CONTACT:
                    ...
                    break;
              /*  case Constant.OptionType.DELETE:
                    callLikeApi(REQ_DELETE, position, Constant.URL_PRODUCT_DELETE, videoList.get(position).getItem());
                    break;*/
                case Constant.OptionType.EDIT:
                    if (videoList.get(position).getType().equals("my_wishlist")) {
                        Map<String, Object> map = new HashMap<>();
                        map.put(Constant.KEY_WISHLIST_ID, videoList.get(position).getItem().getWishListId());
                        fragmentManager.beginTransaction()
                                .replace(R.id.container,
                                        AddToPlaylistFragment.newInstance(Constant.FormType.TYPE_EDIT_WISHLIST, map, Constant.URL_EDIT_WISHLIST))
                                .addToBackStack(null).commit();

                    } else {
                        Map<String, Object> map = new HashMap<>();
                        map.put(Constant.KEY_PRODUCT_ID, videoList.get(position).getItem().getProductId());
                        fragmentManager.beginTransaction()
                                .replace(R.id.container,
                                        CreateEditStoreFragment.newInstance(Constant.FormType.EDIT_PRODUCT, map, Constant.URL_PRODUCT_EDIT, null))
                                .addToBackStack(null)
                                .commit();
                    }
                    break;
    ...
    public void showDeleteDialog(final int position) {
        try {
            ...
            TextView tvMsg = (TextView) progressDialog.findViewById(R.id.tvDialogText);
            if (videoList.get(position).getType().equals("my_wishlist")){
                tvMsg.setText(getStrings(R.string.MSG_DELETE_CONFIRMATION_WISHLIST));
            } else{
                tvMsg.setText(getStrings(R.string.MSG_DELETE_CONFIRMATION_PRODUCT));
            }
            ...
            progressDialog.findViewById(R.id.bCamera).setOnClickListener(v -> {
                progressDialog.dismiss();
                if (videoList.get(position).getType().equals("my_wishlist")) {
                    callLikeApi(REQ_DELETE, position, Constant.URL_DELETE_WISHLIST, -1);
                }else{

                    callLikeApi(REQ_DELETE, position, Constant.URL_PRODUCT_DELETE, -1);
                }
...
//            final StoreContent vo = videoList.get(selectedVo).getProducts().get(position);
            ...
            try {
                ...
                if (videoList.get(position).getType().equals("my_wishlist")){
                    request.params.put(Constant.KEY_WISHLIST_ID, vo.getWishListId());
                }else{
                    request.params.put(Constant.KEY_ID, vo.getProductId());
                    request.params.put(Constant.KEY_PRODUCT_ID, vo.getProductId());
...
//            adapter.notifyItemChanged(selectedVo);
            adapter.notifyDataSetChanged();
