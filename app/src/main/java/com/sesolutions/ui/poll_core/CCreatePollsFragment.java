...
//    private static final int IMAGE = 803;
    ...
    public static CCreatePollsFragment newInstance(int type, Map<String, Object> map, String url) {
        CCreatePollsFragment fragment = new CCreatePollsFragment();
...
//    public static CCreatePollsFragment newInstance(int type, Mapn<String, Object> map, String url, String selectedModule) {
//        CCreatePollsFragment fragment = new CCreatePollsFragment();
//        fragment.FORM_TYPE = type;
//        fragment.url = url;
//        fragment.map = map;
...
//    }
    ...
    public static CCreatePollsFragment newInstance(int formType, String url, OnUserClickedListener<Integer, Object> listener) {
        CCreatePollsFragment frag = new CCreatePollsFragment();
        frag.url = url;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle saveInstanceState) {
        ...
        v = inflater.inflate(R.layout.fragment_create_poll_core, container, false);
        try {
            applyTheme(v);
            initScreenData();
    ...
    private void init() {
        ...
        filedocid = v.findViewById(R.id.filedocid);
        filedisplayid = v.findViewById(R.id.filedisplayid);
        ...
        spPollprivacy = v.findViewById(R.id.sppollprivacy);

        filedocid.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void addOptions(PollOption opt) {
        View view = getLayoutInflater().inflate(R.layout.layout_added_poll_option_core, llOptions, false);
...
//        view.findViewById(R.id.ivGallery).setOnClickListener(v -> onItemClicked(IMAGE, null, currentCount));
                        ...
                        try {
                            ...
                            if (response != null) {
                                ...
                                if (null != result.getFormfields()) {
                                    ...
                                    onBackPressed();
                                    Util.showSnackbar(v, "Sorry, you don't have permission to create a new poll..");
                                }
                                fetchCustomParam();
                                createFormUI();
    ...
    private void fetchCustomParam() {
        try {
            ...
            if (FORM_TYPE == Constant.FormType.EDIT_CORE_POLL) {
            ...
            MAX_OPTION_COUNT = obj.get("maxOptions").getAsInt();
        } catch (Exception e) {
            CustomLog.e(e);
        }
    }
    ...
    private void validateInputDataAndSubmit() {
        ...
        requestpath.put("description", desc);
        //check for options
        ...
        requestpath.put("auth_view", Util.getKeyFromValue(privacyviewOptions, spPollprivacy.getSelectedItem()));
        requestpath.put("auth_comment", Util.getKeyFromValue(privacyMapOptions, spCommentPrivacy.getSelectedItem()));
        requestpath.put("search", cbSearch.isChecked() ? "1" : "0");
...
//
                requestpath.put("optionsArray[" + index + "]", text);
///*
...
//                     }
//                 }
                if (FORM_TYPE == Constant.FormType.EDIT_CORE_POLL && selectedOptionIdMap.containsKey(i)) {
                    requestpath.put("optionIds[" + index + "]", selectedOptionIdMap.get(i));
...
//
    ...
    private void createFormUI() {
        ...
        if (fieldMap.containsKey("auth_view")) {
            v.findViewById(R.id.llpollprivacy).setVisibility(View.VISIBLE);
            privacyviewOptions = fieldMap.get("auth_view").getMultiOptions();
            ((TextView) v.findViewById(R.id.tvpollprivacy)).setText(fieldMap.get("auth_view").getLabel());
            List<String> list = fieldMap.get("auth_view").getMultiOptionsList();
            ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>(context, R.layout.spinner_custom_row, list);
            spPollprivacy.setAdapter(spinnerAdapter);
            //spCommentPrivacy.setSe
        } else {
            v.findViewById(R.id.llpollprivacy).setVisibility(View.GONE);
        }
        ...
        if (fieldMap.containsKey( Constant.FILE)) {
            v.findViewById(R.id.llCommentPrivacy).setVisibility(View.VISIBLE);
            ((TextView) v.findViewById(R.id.tvCommentPrivacy)).setText(fieldMap.get("auth_comment").getLabel());
            List<String> list = fieldMap.get("auth_comment").getMultiOptionsList();
            ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>(context, R.layout.spinner_custom_row, list);
...
//    private Map<Integer, String> selectedGifMap = new HashMap<>();
    ...
    public void onResponseSuccess(int reqCode, Object result) {
        if (null != result) {
            filePathst = ((List<String>) result).get(0);

            requestpath.put(Constant.FILE_TYPE + "image", filePathst);
          ...
          /*  ImageView ivPollImage = llOptions.findViewWithTag(fetchingDataForViewAt).findViewById(R.id.ivPollImage);
            ivPollImage.setVisibility(View.VISIBLE);
            ivPollImage.setImageDrawable(Drawable.createFromPath(filePathst));*/
            filedisplayid.setVisibility(View.VISIBLE);
            filedisplayid.setImageDrawable(Drawable.createFromPath(filePathst));
