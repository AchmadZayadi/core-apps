...
public class StoryPlayerProgressView extends View {
    ...
    public void setProgressBarsCount(int count) {
        ...
        isCancelled = new boolean[progressBarsCount];
        calculateWidthOfEachProgressBar(progressBarsCount);
        invalidate();
        if (storyPlayerListener != null) {
            storyPlayerListener.onPreStartPlaying(0);
        }
    ...
    public void pauseProgress() {
        if (progressAnimator != null) {
            if (progressAnimator.isRunning()) {
                storyPlayerListener.onPausePlaying(currentProgressIndex);
    ...
    public void resumeProgress() {
        if (progressAnimator != null) {
            if (progressAnimator.isPaused()) {
                ...
                storyPlayerListener.onResumePlaying(currentProgressIndex);
            }
        }
    }
   ...
   /* public void cancelAnimation() {
    ...
    private void resetProgressBar(int index, int value) {
      /*  int left = (mGapBetweenProgressBars + singleProgressBarWidth) * i + mGapBetweenProgressBars;
        ...
        progressBarRightEdge[index] = ((index + 1) * mGapBetweenProgressBars + index * singleProgressBarWidth + value) - 1;
        invalidate();
    ...
    public void startProgressFor(final int index) {
        startProgressFor(index, singleStoryDisplayTime);
    }

    public void setCurrentProgressIndex(int currentProgressIndex) {
        this.currentProgressIndex = currentProgressIndex;
    }

    public void startProgressFor(final int index, final long singleStoryDisplayTime) {
        ...
        try {
            ...
            //CustomLog.e("singleProgressBarWidth", "" + singleProgressBarWidth);
            ...
            progressAnimator.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animator) {
                    ...
                    storyPlayerListener.onPreStartPlaying(index + 1);
                }

                @Override
                public void onAnimationCancel(Animator animator) {
                    isCancelled[index] = true;
                    storyPlayerListener.onCancelPlaying(index);
                }
    ...
    //play next if current index is not last
    public void playNext() {
        if (null != progressAnimator)
            progressAnimator.cancel();
        resetProgressBar(currentProgressIndex, singleProgressBarWidth);
        if ((currentProgressIndex + 1) >= progressBarsCount) {
            storyPlayerListener.onFinishedPlaying();
            return;
        }
        storyPlayerListener.onPreStartPlaying(currentProgressIndex + 1);
    }

    //play previous if current index is not first
    public void playPrevious() {
        if (currentProgressIndex != 0) {
            progressAnimator.cancel();
            resetProgressBar(currentProgressIndex, 0);
            storyPlayerListener.onPreStartPlaying(currentProgressIndex - 1);
        }
    ...
    public interface StoryPlayerListener {
        ...
        void onPreStartPlaying(int index);

        void onCancelPlaying(int index);

        void onPausePlaying(int index);

        void onResumePlaying(int index);
