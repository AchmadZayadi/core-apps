...
public class ClassifiedParentFragment extends BaseFragment implements View.OnClickListener, OnUserClickedListener<Integer, Object> {
    ...
    private void setupViewPager() {
        ...
        adapter.addFragment(BrowseClassifiedFragment.newInstance(this, 0), Constant.TAB_TITLE_CLASSIFIED_1);
        if (SPref.getInstance().isLoggedIn(context)) {
            adapter.addFragment(MyClassifiedFragment.newInstance(this, SPref.getInstance().getInt(context, Constant.KEY_LOGGED_IN_ID)), Constant.TAB_TITLE_CLASSIFIED_2);
            adapter.addFragment(
                    CreateClassifiedFragment.newinstance(Constant.FormType.CREATE_CLASSIFIED, Constant.URL_CREATE_CLASSIFIED, this), Constant.TAB_TITLE_CLASSIFIED_3);
        }
    ...
    private void refreshScreenByPosition(int position) {
        ...
        try {
            switch (position) {
                case 0:
                    if (!isBlogLoaded)
                        ((BrowseClassifiedFragment) (adapter.getItem(position))).onRefresh();
                    break;

               /* case 1:
                    if (!isPhotoLoaded)
                        ((BrowsePhotoFragment) adapter.getItem(position)).onRefresh();
                    break;*/

                case 1:
                    if (!isMyAlbumLoaded)
                        ((MyClassifiedFragment) adapter.getItem(position)).onRefresh();
                    break;

                case 2:
                    // if (!isPostVideoLoaded)
                    ((CreateClassifiedFragment) adapter.getItem(position)).initScreenData();
                    //  break;
    ...
    private void loadFragmentIfNotLoaded(int position) {
        ...
        try {
            switch (position) {
                case 0:
                    if (!isBlogLoaded)
                        ((BrowseClassifiedFragment) (adapter.getItem(position))).initScreenData();
                    break;
            ...
            /*    case 1:
                ...
                case 2:
                    if (!isCategoryLoaded)
                        ((AlbumCategoriesFragment) adapter.getItem(position)).initScreenData();
                    break;*/
                case 1:
                    if (!isMyAlbumLoaded)
                        ((MyClassifiedFragment) adapter.getItem(position)).initScreenData();
                    break;
   ...
   /* private void goToFormFragment() {
    ...
    private void goToSearchFragment() {
        fragmentManager.beginTransaction().replace(R.id.container, new SearchClassifiedFragment()).addToBackStack(null).commit();
    }
    ...
    public boolean onItemClicked(Integer object1, Object object2, int postion) {
        if (object1 == Constant.Events.SUCCESS) {
            isMyAlbumLoaded = false;
            tabLayout.getTabAt(0).select();
            goToViewClassifiedFragment(postion);
        }
        return false;
    }
}
