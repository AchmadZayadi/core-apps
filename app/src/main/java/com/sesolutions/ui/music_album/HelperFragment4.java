...
public class HelperFragment4 extends CommentLikeHelper {

    public MusicParentFragment parent;
    public List<Albums> albumsList;
    public MusicLyricsAdapter adapter;
        ...
        try {
            switch (object1) {
                ...
                case Constant.Events.MUSIC_MAIN:
                    goToNextScreen(postion, (MusicLyricsAdapter.ContactHolder) screenType);
                    break;
            }
    ...
    private void goToNextScreen(int screenType, MusicLyricsAdapter.ContactHolder holder) {
        ...
        switch (screenType) {
            case Constant.FormType.TYPE_SONGS:
                Map<String, Object> map = new HashMap<>();
                goToSongsView(map, albumsList.get(position).getSongId(), albumsList.get(position).getResourceType(), holder, bundle,1);
                break;
            case Constant.FormType.TYPE_LYRICS:
                map = new HashMap<>();
                map.put(Constant.KEY_LYRICS, 1);
                goToSongsView(map, albumsList.get(position).getSongId(), albumsList.get(position).getResourceType(), holder, bundle,2);
                break;
    ...
    private void goToViewMusicAlbumFragment(int position, MusicLyricsAdapter.ContactHolder holder, Bundle bundle) {
    ...
    private void goToViewPlaylistFragment(int postion, MusicLyricsAdapter.ContactHolder holder, Bundle bundle) {
        try {
            fragmentManager.beginTransaction()
                    .addSharedElement(holder.ivSongImage, ViewCompat.getTransitionName(holder.ivSongImage))
    ...
    private void goToViewArtistFragment(int postion, MusicLyricsAdapter.ContactHolder holder, Bundle bundle) {
        try {
            fragmentManager.beginTransaction()
                    .addSharedElement(holder.ivSongImage, ViewCompat.getTransitionName(holder.ivSongImage))
                    .addSharedElement(holder.ivFavorite, ViewCompat.getTransitionName(holder.ivFavorite))
    ...
    private void goToSongsView(Map<String, Object> map, int songId, String resourceType, MusicLyricsAdapter.ContactHolder holder, Bundle bundle,int TypeFlag) {
        ...
        try {
            if(TypeFlag==1){
                fragmentManager.beginTransaction()
                        ...
                        //   .addSharedElement(holder.llMain, ViewCompat.getTransitionName(holder.llMain))
                        .addSharedElement(holder.tvSongTitle, ViewCompat.getTransitionName(holder.tvSongTitle))
                        .replace(R.id.container, ViewSongFragment.newInstance(map, songId, resourceType, bundle,TypeFlag)).addToBackStack(null).commit();
            }else {
                fragmentManager.beginTransaction()
                        .addSharedElement(holder.ivSongImage, ViewCompat.getTransitionName(holder.ivSongImage))
                        //   .addSharedElement(holder.llMain, ViewCompat.getTransitionName(holder.llMain))
