...
public class RSSHelper extends SpeakableContent {
    ...
    public boolean onItemClicked(Integer object1, Object screenType, int postion) {
        ...
        switch (object1) {
            ...
            case Constant.Events.FEED_UPDATE_OPTION:
                ...
                performFeedOptionClick(videoList.get(Integer.valueOf("" + screenType)).getRSSId(), vo, Integer.parseInt("" + screenType), postion);
                break;
            ...
            case Constant.Events.MUSIC_MAIN:
                goToNextScreen((RSSAdapter.ContactHolder) screenType);
                break;
    ...
    private void performFeedOptionClick(int rssId, Options vo, int actPosition, int position) {
        switch (vo.getName()) {
            case Constant.OptionType.DELETE:
                showDeleteDialog(1, rssId, actPosition);
                //callSaveFeedApi(REQ_CODE_OPTION_DELETE, Constant.URL_FEED_DELETE, actionId, vo, actPosition, position);
                break;
            case Constant.OptionType.EDIT:
                goToFormFragment(rssId);
                // callSaveFeedApi(REQ_CODE_OPTION_SAVE, Constant.URL_FEED_SAVE, actionId, vo, actPosition, position);
                break;
        }
    }

    public void showDeleteDialog(final int REQ, final int rssId, final int position) {
        try {
            ...
            tvMsg.setText(R.string.MSG_DELETE_CONFIRMATION_RSS);
            ...
            progressDialog.findViewById(R.id.bCamera).setOnClickListener(v -> {
                progressDialog.dismiss();
                callDeleteApi(REQ, rssId, position);
            });
    ...
    private void callDeleteApi(final int REQ, final int rssId, final int position) {
        ...
        try {
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_DELETE_RSS);
                    request.params.put(Constant.KEY_RSS_ID, rssId);
    ...
    private void goToFormFragment(int screenType, int position) {
        ...
        if (screenType == Constant.FormType.TYPE_MUSIC_ALBUM) {
            ...
            map.put(Constant.KEY_RSS_ID, videoList.get(position).getRSSId());
        } /*else {
    ...
    public void goToFormFragment(int rssId) {
        Map<String, Object> map = new HashMap<>();
        map.put(Constant.KEY_RSS_ID, rssId);
        // map.put(Constant.KEY_GET_FORM, 1);
        fragmentManager.beginTransaction().replace(R.id.container, FormFragment.newInstance(Constant.FormType.TYPE_RSS_EDIT, map, Constant.URL_EDIT_RSS)).addToBackStack(null).commit();
    }


    private void goToNextScreen(RSSAdapter.ContactHolder holder) {
        ...
        try {
            ...
            fragmentManager.beginTransaction()
                    ...
                    .replace(R.id.container, ViewRSSFragment.Companion.newInstance(videoList.get(position).getRSSId(), bundle))
                    .addToBackStack(null).commit();
        } catch (Exception e) {
            CustomLog.e(e);
            fragmentManager.beginTransaction()
                    .replace(R.id.container, ViewRSSFragment.Companion.newInstance(videoList.get(position).getRSSId()))
                    .addToBackStack(null).commit();
        }

    }

   /* private void goToViewRSSFragment(int postion) {
        fragmentManager.beginTransaction()
                .replace(R.id.container
                        , ViewRSSFragment.newInstance(videoList.get(postion).getRSSId()))
                .addToBackStack(null)
                .commit();
    }*/

    private void updateItemLikeFavorite(int REQ_CODE, int position, RSS vo) {
    ...
    private void callLikeApi(int screenType, final int REQ_CODE, final int position, String url, final RSS vo) {
    ...
    public String getDetail(RSS album) {
        String detail = "";
