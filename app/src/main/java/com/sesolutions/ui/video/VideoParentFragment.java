...
public class VideoParentFragment extends BaseFragment implements View.OnClickListener, OnUserClickedListener<Integer, Object> {
    ...
    public static VideoParentFragment newInstance(int index) {
        VideoParentFragment frag = new VideoParentFragment();
    ...
    private void setupViewPager() {
        ...
        if(!SPref.getInstance().isBasicPlugins(context,"video")){
            tabItems.add("channel");
            tabItems.add("playlist");
            tabItems.add("artist");
            tabItems.add("categories");
        }
        /*if (!isCoreVideoPluginEnabled) {
        }*/
        ...
        for (String name : tabItems) {
            switch (name) {
                ...
                case "browse":
                    adapter.addFragment(VideoAlbumFragment.newInstance(this, name), getStrings(R.string.TAB_TITLE_VIDEO_1));
                    break;
                case "channel":
                    adapter.addFragment(VideoChannelFragment.newInstance(this, name), getStrings(R.string.TAB_TITLE_VIDEO_2));
                    break;
                case "playlist":
                    adapter.addFragment(VideoPlaylistFragment.newInstance(this, name), getStrings(R.string.TAB_TITLE_VIDEO_3));
                    break;
                case "artist":
                    adapter.addFragment(VideoArtistsFragment.newInstance(this, name), getStrings(R.string.TAB_TITLE_VIDEO_4));
                    break;
                case "categories":
                    adapter.addFragment(VideoCategoriesFragment.newInstance(this, name), getStrings(R.string.TAB_TITLE_VIDEO_5));
                    break;
                case "myVideos":
                    adapter.addFragment(MyVideoFragment.newInstance(this, name), getStrings(R.string.TAB_TITLE_VIDEO_6));
                    break;
                case "createVideos":
                    adapter.addFragment(PostVideoForm.newinstance(Constant.FormType.CREATE_VIDEO, Constant.URL_CREATE_VIDEO, this), getStrings(R.string.TAB_TITLE_VIDEO_7));
                    break;
            }
        }
    ...
    private void goToSearchFragment() {
        try {
            ...
            }else if ("artist".equals(tabItems.get(pos))) {
                frag = new SearchVideoArtistsFragment();
            }
    ...
    public void onResume() {
        super.onResume();
        if (BaseActivity.backcoverchange == Constant.FormType.CREATE_VIDEO) {
             BaseActivity.backcoverchange=0;
            adapter.getItem(1).initScreenData();
    ...
    public void onStart() {
        ...
        try {
            ...
            }else if (activity.taskPerformed == Constant.FormType.DELETE_CHANNEL) {
              /*  this means current selected fragment is "My Video-> Playlist"
                So update the playlist */
                activity.taskPerformed = 0;
             //   tabLoaded[getTabIndex("myVideo")] = false;
                adapter.getItem(getTabIndex("channel")).initScreenData();
