...
public class GroupInfoFragment2 extends BaseFragment implements View.OnClickListener {
    ...
    public static GroupInfoFragment2 newInstance(int userId, boolean showToolbar) {
        GroupInfoFragment2 frag = new GroupInfoFragment2();
        frag.mPageId = userId;
        frag.showToolbar = showToolbar;
        return frag;
    }

    public static GroupInfoFragment2 newInstance(int userId) {
        GroupInfoFragment2 frag = new GroupInfoFragment2();
        frag.mPageId = userId;
        return frag;
    }
    ...
    private void addPeopleLists(String s, List<Albums> list, int dImage, int total, String type) {
        try {
            ...
            if (list.size() > 1) {
                ...
                final int pageId = list.get(1).getGroupId();
                item1.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (userId > 0) {
    ...
    private void setBasicInfoData() {
        ...
        try {
            ...
            if(resp.getOwner_name()!=null && resp.getOwner_name().length()>0){
                View   view = getLayoutInflater().inflate(R.layout.layout_text_horizontal, (ViewGroup) llBasic, false);
                ((TextView) view.findViewById(R.id.tv1)).setText("Owner name");
                ((TextView) view.findViewById(R.id.tv2)).setText(resp.getOwner_name());
            ...
            case Constant.OptionType.OWNERNAME:
            ...
            if (resp.getBasicInformation() != null) {
                ...
                if(resp.getDescription()!=null && resp.getDescription().length()>0){
                    View   view = getLayoutInflater().inflate(R.layout.layout_text_horizontal, (ViewGroup) llBasic, false);
                    ((TextView) view.findViewById(R.id.tv1)).setText("Description");
                    ((TextView) view.findViewById(R.id.tv2)).setText(Html.fromHtml(resp.getDescription()));
                    llBasic.addView(view);
                }
               try {
                    if(resp.getTags()!=null && resp.getTags().size()>0){
                        View   view = getLayoutInflater().inflate(R.layout.layout_text_horizontal, (ViewGroup) llBasic, false);
                        ((TextView) view.findViewById(R.id.tv1)).setText("Tags");
                        ((TextView) view.findViewById(R.id.tv2)).setText(Html.fromHtml(resp.getTags().get(0).getText()));
                        llBasic.addView(view);
                    }
                }catch (Exception ex){
                    ex.printStackTrace();
            ...
            for (final NestedOptions opt : resp.getContactInformation()) {
                switch (opt.getName()) {
                    ...
                    case Constant.OptionType.CREATE_DATE:
                        ...
                        break;
                    default:
                        if (opt.getValueString()!=null && opt.getValueString().length()>0){
                            view = getLayoutInflater().inflate(R.layout.layout_text_image_horizontal, (ViewGroup) llAbout, false);
        ...
        try {
            ...
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    request.params.put(Constant.KEY_GROUP_ID, mPageId);
