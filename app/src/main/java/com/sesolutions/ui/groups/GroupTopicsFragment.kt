...
open class GroupTopicsFragment : BaseFragment(), androidx.swiperefreshlayout.widget.SwipeRefreshLayout.OnRefreshListener, OnUserClickedListener<Int, Any>, OnLoadMoreListener, View.OnClickListener {
    ...
    lateinit var adapter: GroupTopicAdapter

    private lateinit var forumList: List<ForumResponse.ForumContent>
    lateinit var topics: MutableList<ForumResponse.Topic>
    ...
    companion object {
        ...
        fun newInstance(forumId: Int): GroupTopicsFragment {
        ...
        fun newInstance(forumId: Int, fromGroup: Boolean): GroupTopicsFragment {
    ...
    private fun updateModerator(moderator: ForumResponse.Moderator) {
        ...
        if (moderator.moderators != "") {
            v!!.findViewById<View>(R.id.llModerator).visibility = View.GONE
    ...
    fun updateButton(){
        ...
        if (SPref.getInstance().isLoggedIn(context)) {
            if (null != result!!.button) {
                llCreateTopic!!.visibility = View.VISIBLE
                tvPost!!.text = "Create New Topic"
            } else {
    ...
    fun setRecyclerView() {
        try {
            ...
            adapter = GroupTopicAdapter(topics, context, this, this)
    ...
    override fun onItemClicked(eventType: Int?, data: Any, position: Int): Boolean {
        ...
        when (eventType) {
            Constant.Events.TOPIC_OPTION -> ForumUtil.openViewGroupTopicFragment(fragmentManager, position)
            ...
            when (v.id) {
                ...
                R.id.llCreateTopic -> {
                    if (SPref.getInstance().isLoggedIn(context)) {
                        ...
                        map[Constant.KEY_GROUP_ID] = forumId
                        openFormFragment(Constant.FormType.CREATE_FORUM_TOPIC, map, Constant.URL_GROUP_TOPIC_CREATE)
                    } else {
