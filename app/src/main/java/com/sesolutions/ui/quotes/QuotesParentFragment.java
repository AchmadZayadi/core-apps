...
public class QuotesParentFragment extends BaseFragment implements View.OnClickListener, OnUserClickedListener<Integer, Object> {
    ...
    private void setupViewPager() {
        ...
        adapter.addFragment(BrowseQuotesFragment.newInstance(this, 0), Constant.TAB_TITLE_QUOTES_1);
        ...
        if (SPref.getInstance().isLoggedIn(context)) {
            ...
            adapter.addFragment(ManageQuotesFragment.newInstance(this, 0), Constant.TAB_TITLE_QUOTES_2);
            adapter.addFragment(
                    CreateQuoteFragment.newinstance(Constant.FormType.CREATE_QUOTE, Constant.URL_CREATE_QUOTE, this), Constant.TAB_TITLE_QUOTES_4);
        }
    ...
    private void refreshScreenByPosition(int position) {
        ...
        try {
            switch (position) {
                case 0:
                    if (!isQuoteLoaded)
                        ((BrowseQuotesFragment) (adapter.getItem(position))).onRefresh();
                    break;

                case 1:
                    if (!isMyQuoteLoaded)
                        ((ManageQuotesFragment) adapter.getItem(position)).onRefresh();
                    break;
    ...
    private void loadFragmentIfNotLoaded(int position) {
        ...
        try {
            switch (position) {
                case 0:
                    if (!isQuoteLoaded)
                        ((BrowseQuotesFragment) (adapter.getItem(position))).initScreenData();
                    break;

                case 1:
                    if (!isMyQuoteLoaded)
                        ((ManageQuotesFragment) adapter.getItem(position)).initScreenData();
                    break;
