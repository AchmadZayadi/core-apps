...
public class UpcomingEventFragment extends EventHelper<EventAdapter> implements OnLoadMoreListener {
    ...
    public static UpcomingEventFragment newInstance(OnUserClickedListener<Integer, Object> parent, int loggedInId, int categoryId) {
        UpcomingEventFragment frag = new UpcomingEventFragment();
        frag.parent = parent;
    ...
    public static UpcomingEventFragment newInstance(OnUserClickedListener<Integer, Object> parent, int loggedInId) {
        return newInstance(parent, loggedInId, -1);
    }

    public static UpcomingEventFragment newInstance(String TYPE, OnUserClickedListener<Integer, Object> parent) {
        UpcomingEventFragment frag = newInstance(parent, -1, -1);
        frag.selectedScreen = TYPE;
        return frag;
    }


    public static UpcomingEventFragment newInstance(String TYPE, int listId) {
        UpcomingEventFragment frag = newInstance(null, -1, -1);
        frag.selectedScreen = TYPE;
        frag.listId = listId;
        return frag;
    }
    ...
    public static UpcomingEventFragment newInstance(int categoryId) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle saveInstanceState) {
        ...
        v = inflater.inflate(selectedScreen.equals(TYPE_BROWSE_LIST) || selectedScreen.equals(TYPE_BROWSE_HOST) ? R.layout.fragment_list_common_offset_refresh : R.layout.fragment_common_list_refresh, container, false);
        txtNoData = "  ";
        applyTheme(v);
        return v;
    }
    ...
    public void init() {
        try {
            ...
            switch (selectedScreen) {
                ...
                case TYPE_MANAGE:
                    url = Constant.URL_MANAGE_EVENT;
                    break;
                case TYPE_BROWSE_LIST:
                    url = Constant.URL_LIST_EVENT;
                    break;
                case TYPE_BROWSE_HOST:
                    url = Constant.URL_EVENT_HOST;
                    break;
                case TYPE_VIEW_CATEGORY:
                    url = Constant.URL_CATEGORY_VIEW_EVENT;
                    break;
                default:
                    url = Constant.URL_UPCOMING_EVENT;
                    break;
            }
        } catch (Exception e) {
            CustomLog.e(e);
    ...
    public void setRecyclerView() {
        try {
            ...
            if (selectedScreen.equals(TYPE_BROWSE_LIST) || selectedScreen.equals(TYPE_BROWSE_HOST)) {
                layoutManager = new StaggeredGridLayoutManager(Constant.SPAN_COUNT, StaggeredGridLayoutManager.VERTICAL);
            } else {
                layoutManager = new LinearLayoutManager(context);
            }
            recyclerView.setLayoutManager(layoutManager);
            ((SimpleItemAnimator) recyclerView.getItemAnimator()).setSupportsChangeAnimations(false);
            adapter = new EventAdapter(videoList, context, this, this);
        ...
        try {
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    if (listId > 0) {
                        request.params.put(Constant.KEY_LIST_ID, listId);
                    }
        ...
        switch (object1) {
            case Constant.Events.FEED_UPDATE_OPTION:
                ...
                if (listPosition > -1) {
                    ...
                    Options opt = videoList.get(listPosition).getOptions().get(postion);
