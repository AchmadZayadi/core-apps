...
public class ViewAlbumFragment extends CommentLikeHelper implements View.OnClickListener, OnLoadMoreListener, PopupMenu.OnMenuItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public TextView tvDescription,tvLocation;
    ...
    public static ViewAlbumFragment newInstance(int albumId, boolean openComment, Bundle bundle) {
    ...
    public static ViewAlbumFragment newInstance(int albumId) {
        return ViewAlbumFragment.newInstance(albumId, false, null);
    }

    public static ViewAlbumFragment newInstance(int albumId, boolean openComment) {
        return ViewAlbumFragment.newInstance(albumId, openComment, null);
    }

    public static ViewAlbumFragment newInstance(boolean uploadPhotos, int albumId) {
        ViewAlbumFragment frag = new ViewAlbumFragment();
        frag.albumId = albumId;
    ...
    public static ViewAlbumFragment newInstance(int albumId, Bundle bundle) {
        return ViewAlbumFragment.newInstance(albumId, false, bundle);
    }
            ...
            switch (item.getItemId()) {
                ...
                case R.id.option:
                    if (null != result) {
                        ...
                        showPopupalubum(result.getMenus(), vItem, 10,album.getOwnerId());
                    }
                    break;
            }
        } catch (Exception e) {
    ...
    private void updateUpperLayout() {
        try {
            ...
            if (null == bundle)
                Util.showImageWithGlide(ivCoverPhoto1, album.getFirstCover(), context, R.drawable.placeholder_square);
            ...
            try {
                tvLocation.setText(album.getLocation_ad());
                tvLocation.setVisibility(album.getLocation_ad().length()>0? View.VISIBLE:View.GONE);
            }catch (Exception ex){
                ex.printStackTrace();
                tvLocation.setVisibility(View.GONE);
            }
           tvDescription.setVisibility(album.getDescription().length()>0? View.VISIBLE:View.GONE);
            Util.showImageWithGlide(ivUserImage, album.getUserImage(), context, R.drawable.placeholder_square);
            tvAlbumDetail.setText(getDetail(album,result.getPhotos().size()));
    ...
    public String getDetail(Albums album,int photolength) {
        String detail = "";
        ...
        if(photolength>0){
            detail += "\uf164 " + album.getLikeCount()// + (album.getLikeCount() != 1 ? " Likes" : " Like")
                    ...
                    + "  \uf03e " + photolength;// + (album.getSongCount() > 1 ? " Songs" : " Song");
        }else {
            detail += "\uf164 " + album.getLikeCount()// + (album.getLikeCount() != 1 ? " Likes" : " Like")
                    + "  \uf075 " + album.getCommentCount() //+ (album.getCommentCount() != 1 ? " Comments" : " Comment")
                    + "  \uf004 " + album.getFavouriteCount() //+ (album.getFavouriteCount() != 1 ? " Favorites" : " Favorite")
    ...
    private void showShareDialog(String msg) {
        try {
            ...
            progressDialog.setContentView(R.layout.dialog_message_three2);
            ...
            bShareIn.setText(getStrings(R.string.TXT_SHARE_INSIDE) + AppConfiguration.SHARE);
    ...
    public void onClick(View view) {
        ...
        try {
            switch (view.getId()) {
                ...
                case R.id.ivOption:
                    showPopupalubum(result.getMenus(), view, 10,album.getOwnerId());
                    break;

                case R.id.ivCamera:
                    showPopupalubum(album.getCoverImageOptions(), v.findViewById(R.id.ivCamera), 100,album.getOwnerId());
                    break;

                case R.id.tvUserTitle:
                case R.id.ivUserImage:
                    goTo(Constant.GoTo.PROFILE, Constant.KEY_ID, album.getOwnerId());
                    break;
    ...
    private void showPopupalubum(List<Options> menus, View v, int idPrefix,int userid) {
        if (null != menus && menus.size() > 0) {
            try {
                PopupMenu menu = new PopupMenu(context, v);
                for (int index = 0; index < menus.size(); index++) {
                    ...
                    Log.e("userid",""+userid);
                    Log.e("userid2",""+SPref.getInstance().getLoggedInUserId(getActivity()));

                    if(userid==SPref.getInstance().getLoggedInUserId(getActivity())){
                        if(!s.getName().equalsIgnoreCase("report")){
                            menu.getMenu().add(1, idPrefix + index + 1, index + 1, s.getLabel());
                        }
                    }else {
                        menu.getMenu().add(1, idPrefix + index + 1, index + 1, s.getLabel());
        ...
        try {
            ...
            switch (opt.getName()) {
                ...
                case Constant.OptionType.CHOOSE_FROM_ALBUMS:
                    ...
                    openSelectAlbumFragment(Constant.URL_ALBUM_UPLOAD_COVER_TAG, map);
                    break;
    ...
    private void gToAlbumImage(String url, String main, String title) {
        ...
        map.put(Constant.KEY_ALBUM_ID, album.getAlbumId());
        fragmentManager.beginTransaction()
                .replace(R.id.container, AlbumImageFragment.newInstance(title, url, main, map,true))
                .addToBackStack(null)
                .commit();
    }
    ...
    private void openLighbox(int photoId, String imageUrl) {
        ...
        intent.putExtra(Constant.KEY_URI, imageUrl);
        intent.putExtra(Constant.KEY_ID, photoId);
        intent.putExtra(Constant.KEY_TYPE, Constant.ACTIVITY_TYPE_ALBUM);
        intent.putExtra(Constant.KEY_RESOURCES_TYPE,  Constant.ResourceType.ALBUM_PHOTO);
