...
public class CourseInfoFragment extends BaseFragment {
    ...
    public static CourseInfoFragment newInstance(int userId) {
        CourseInfoFragment frag = new CourseInfoFragment();
        frag.mStoreId = userId;
        return frag;
    }

    boolean isToolbar=false;
    public static CourseInfoFragment newInstance(int userId,boolean isToolbar) {
        CourseInfoFragment frag = new CourseInfoFragment();
        frag.mStoreId = userId;
    ...
    private void init() {
        ...
        try {
            ...
            if (resp != null) {
                ...
                addPeopleLists(getStrings(R.string.businesses_liked), resp.getLikeBusinesses(), R.drawable.music_like_selected, resp.getTotalLikeBusinesses(), "business");
    ...
    private void addPeopleLists(String s, List<Albums> list, int dImage, int total, String type) {
        try {
            ...
            if (list.size() > 0) {
                ...
                final int storeId = list.get(0).getStoreId();
...
//                            StoreUtil.openViewStoreFragment(fragmentManager, storeId);
//                            openViewBusinessFragment(businessId);
                        }
                    }
                });
            ...
            if (list.size() > 1) {
                ...
                final int businessId = list.get(1).getBusinessId();
                item1.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (userId > 0) {
            ...
            if (list.size() > 2) {
                ...
                final int businessId = list.get(2).getBusinessId();
            ...
            if (list.size() > 3) {
                ...
                final int businessId = list.get(3).getBusinessId();
                item1.setOnClickListener(v -> {
                    if (userId > 0) {
                        goToProfileFragment(userId);
                    } else {
    ...
    private void openMoreMemberFragment(String type) {
        ...
        bundle.putInt(Constant.KEY_ID, mStoreId);
        ...
        try {
            ...
            if (isNetworkAvailable(context)) {
                ...
                try {
                    ...
                    HttpRequestVO request = new HttpRequestVO(Constant.URL_COURSE_INFO);
                    request.params.put(Constant.KEY_COURSE_ID, mStoreId);
