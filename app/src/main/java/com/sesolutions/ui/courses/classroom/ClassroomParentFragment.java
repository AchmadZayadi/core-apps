...
public class ClassroomParentFragment extends GlobalTabHelper {
...
//            loadFragmentIfNotLoaded(0);
    ...
    public void init() {
        ...
        if (SPref.getInstance().isLoggedIn(context)) {
            ...
            v.findViewById(R.id.ivAccount).setVisibility(View.GONE);
        }
        initButtons();
    }
    ...
    public void setupViewPager() {
        ...
        if (result.CanCreateClassroom() && SPref.getInstance().isLoggedIn(context)) {
        ...
        for (Options opt : tempMenu) {
            switch (opt.getAction()) {
                case ClassroomFragment.TYPE_HOME:
                    break;

                case "eclassroom_main_browse":
                    adapter.addFragment(ClassroomFragment.newInstance(opt.getAction(), this), opt.getLabel());
                    break;
                case MenuTab.Store.CLASS_CAT:
                    adapter.addFragment(ClassroomCategoriesFragment.newInstance(this), "Browse Categories");
                    break;
//                case ClassroomFragment.TYPE_ALBUM_BROWSE:
//                    tabItems.add(opt);
//                    HashMap<String, Object> map = new HashMap<>();
//                    map.put(Constant.KEY_URI, Constant.URL_PAGE_BROWSE_ALBUM);
//                    map.put(Constant.SELECT, ClassroomFragment.TYPE_ALBUM_BROWSE);
...
//                    break;
    ...
    public void updateToolbarIcons(int position) {
        ...
        if (canShowCreate(tabItems.get(position).getAction())) {
            fab.setImageResource(ClassroomFragment.TYPE_PACKAGE.equals(tabItems.get(selectedItem).getAction()) ? R.drawable.option_edit : R.drawable.add_24dp);
//            updateFabColor(fab);
            fab.show();
        } else {
            fab.hide();
    ...
    private boolean canShowSearch(String name) {
        switch (name) {
            case ClassroomFragment.TYPE_MANAGE:
            case ClassroomFragment.TYPE_REVIEW_BROWSE:
            case MenuTab.Store.CLASS_CAT:
...
//        super.updateFabColor(v.findViewById(R.id.fabAdd));
    ...
    private void showBottomOptions() {
        ...
        this.v.findViewById(R.id.ll4).setVisibility(getTotal(getTabIndex(ClassroomFragment.TYPE_PACKAGE)) > 0 ? View.VISIBLE : View.GONE);
...
//        fragmentManager.beginTransaction()
//                .replace(R.id.container,
//                        CreateEditClassroomFragment.newInstance(Constant.FormType.CREATE_CLASSROOM, map, Constant.URL_CREATE_CLASSROOM,null))
//                .addToBackStack(null)
//                .commit();
        fetchFormData();
    }
    ...
    public void goToSearchFragment() {
        fragmentManager.beginTransaction().replace(R.id.container, new SearchClassroomFragment()).addToBackStack(null).commit();
    }
                                ...
                                if (err.isSuccess()) {
                                    ...
                                    if (resp != null && resp.getResult() != null && resp.getResult().arePackagesAvailabel()) {
                                        ...
                                        if (vo != null && vo.getResult() != null && vo.getResult().getFormfields() != null) {
                                            openClassroomCreateForm(vo.getResult(), new HashMap<String, Object>());
                                        }
                                    }
                                } else {
