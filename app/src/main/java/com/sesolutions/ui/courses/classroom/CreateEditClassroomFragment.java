...
public class CreateEditClassroomFragment extends FormHelper implements View.OnClickListener {//}, ParserCallbackInterface {

    private AppCompatTextView tvTitle;
    private Dummy.Result result;

   /* public static CreateEditStoreFragment newInstance(int formType, String url, int categoryId) {
        CreateEditStoreFragment frag = new CreateEditStoreFragment();
        frag.url = url;
    ...
    public static CreateEditClassroomFragment newInstance(int type, Map<String, Object> map, String url, Dummy.Result result) {
        CreateEditClassroomFragment fragment = new CreateEditClassroomFragment();
    ...
    private void init() {
        ...
        ((TextView) v.findViewById(R.id.tvTitle)).setText(getStrings(FORM_TYPE == Constant.FormType.CREATE_CLASSROOM ? R.string.title_create_classroom : R.string.title_edit_classroom));
    ...
    public void onValueChanged(BaseFormElement baseFormElement) {
        ...
        try {
            if (BaseFormElement.TYPE_PICKER_SINGLE == baseFormElement.getType()) {
                if (((FormElementPickerSingle) baseFormElement).getName().equals("can_joins")) {
                    ...
                    if (null != key) {
                        ...
                        for (int i = 0; i < tagList.size(); i++) {
                            ...
                            if (name.equals("member_title_plurals") || name.equals("member_title_singulars")) {
    ...
    private void hideInitially() {
        for (int i = 0; i < tagList.size(); i++) {
            ...
            //key = tagList.get(i);
            String name = tagList.get(i);
            if (name.equals("member_title_singulars") || name.equals("member_title_plurals")) {
