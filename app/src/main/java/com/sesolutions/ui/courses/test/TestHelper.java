...
public class TestHelper<T extends RecyclerView.Adapter> extends CommentLikeHelper implements View.OnClickListener {
        ...
        try {
            switch (object1) {
                ...
                case Constant.Events.MUSIC_MAIN:
                    openViewTestFragment(videoList.get(postion).getItem().getTest_id());
                    break;
                case Constant.Events.GIVE_TEST:
                    openGiveTestFragment(videoList.get(postion).getItem().getTest_id());
                    break;
                case Constant.Events.RESULT:
                    openViewTestResultFragment(videoList.get(postion).getItem().getTest_id(), videoList.get(postion).getItem().getUserTest_id());
                    break;
                ...
                case Constant.Events.PAGE_SUGGESTION_MAIN:
                    openViewTestFragment(videoList.get(postion).getTest().getTest_id());
                    break;
...
//                                        }
    ...
    private void callLikeApi(final int REQ_CODE, final int position, String url, final int optionPosition) {
        ...
        if (isNetworkAvailable(context)) {
            final TestContent vo = videoList.get(position).getItem();
    ...
    public void updateItemLikeFavorite(int REQ_CODE, int position, TestContent vo, boolean showAnimation) {
