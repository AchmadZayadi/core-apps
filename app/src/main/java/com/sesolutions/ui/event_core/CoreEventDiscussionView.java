...
public class CoreEventDiscussionView extends BaseFragment implements View.OnClickListener, OnLoadMoreListener, OnUserClickedListener<Integer, Object>, SwipeRefreshLayout.OnRefreshListener {
    ...
    public static CoreEventDiscussionView newInstance(int topicId) {
        CoreEventDiscussionView frag = new CoreEventDiscussionView();
  ...
  /*  private void getMapValues() {
    ...
    private void callMusicAlbumApi(final int req) {
        ...
        if (isNetworkAvailable(context)) {
            ...
            try {
                HttpRequestVO request = new HttpRequestVO(Constant.URL_CEVENT_DISCUSSION_VIEW);
        ...
        switch (object1) {
            ...
            case Constant.Events.FEED_UPDATE_OPTION:
                ...
                if (Constant.OptionType.EDIT.equals(opt.getName())) {
                    ...
                    super.openFormFragment(Constant.FormType.EDIT_TOPIC, map, Constant.URL_EDIT_CORE_EVENT_POST);
                } else {
            ...
            progressDialog.findViewById(R.id.bCamera).setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (isNetworkAvailable(context)) {
                        ...
                        new ApiController(Constant.URL_DELETE_CORE_EVENT_POST, map, context, CoreEventDiscussionView.this, -3).execute();
    ...
    private void handleOptions(String name) {
        ...
        if (isNetworkAvailable(context)) {
            ...
            if (Constant.OptionType.POST_REPLY.equals(name)) {
                openFormFragment(Constant.FormType.REPLY_TOPIC, map, Constant.URL_CORE_EVENT_REPLY_TOPIC);
            } else {
                map.put("watch", Constant.OptionType.STOP_WATCH_TOPIC.equals(name) ? 1 : 0);
                showBaseLoader(true);
                new ApiController(Constant.URL_CORE_EVENT_TOPIC_WATCH, map, context, this, -2).execute();
            }
        } else {
            notInternetMsg(v);
        }
