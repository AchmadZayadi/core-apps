...
public class MediaPickerFragment extends BaseFragment {
    ...
    private void initView(View view) {
        ...
        if (PickerManager.getInstance().showVideo() && adapter.getCount() == 1)
            FilePickerActivity.isVideoCamera = true;
        else if (PickerManager.getInstance().showImages() && adapter.getCount() == 1)
            FilePickerActivity.isVideoCamera = false;

        tabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            @Override
            public void onTabSelected(TabLayout.Tab tab) {
                if (PickerManager.getInstance().showImages() && PickerManager.getInstance().showVideo()) {
